	...
	use MPI
	use Global, only: N1D,Nsites,wmin,wmax,Nw,dw,w,G,H,ind,mu,t1,t2,t3,T,pi,ii,nproc
        implicit none
...
! Declaration of variables:
	...
	real*8 :: sigma2(0:2*Nw),eps1(0:2*Nw),eps2(0:2*Nw),LF(0:2*Nw)
        real*8 :: dummy_xx,dummy_yy,dummy_xy,re,im
	...
	do n=0,N1D-1
		...
		do np=0,N1D-1		
			if ((abs(l-lp)==1).and.(abs(m-mp)==0).and.(abs(n-np)==0)) then
				vxmat(ind(l,m,n),ind(lp,mp,np))=sign(1,l-lp)*ii*H(ind(l,m,n),ind(lp,mp,np))	
			end if
			if ((abs(l-lp)==0).and.(abs(m-mp)==1).and.(abs(n-np)==0)) then
				vymat(ind(l,m,n),ind(lp,mp,np))=sign(1,m-mp)*ii*H(ind(l,m,n),ind(lp,mp,np))	
			end if
		end do
		end do
		end do
